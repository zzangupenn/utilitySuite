...
class PositionalEncoding():
    ...
    def batch_encode(self, batch, loop_ind=1):
        ...
        batch_encoded = np.stack(batch_encoded_list)
...
class PositionalEncoding_torch():
    ...
    def batch_encode2(self, batch, loop_ind=1):
        batch_encoded_list = []
        for ind in range(batch.shape[loop_ind]):
            encoded_ = self.encode(batch[:, ind, None])
            batch_encoded_list.append(encoded_[0])
    ...
    def batch_decode2(self, list_data):
        ...
        list_data = list_data.view(list_data.shape[0], dim, 2)
        atan2_value = torch.arctan2(list_data[:, :, 0], list_data[:, :, 1]) / (self.pi)
        sub_zero_inds = torch.where(atan2_value < 0)
        atan2_value[sub_zero_inds] = atan2_value[sub_zero_inds] + 1
        return atan2_value
